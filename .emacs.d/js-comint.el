...
;; Modified to use node rather than rhino
;; http://js-comint-el.sourceforge.net
(defun js-remove-last-char (foo)
  (substring foo 0 (- (length foo) 1)))
(setq js-prog "node") ;; Modified
(setq js-shell-path (js-remove-last-char (shell-command-to-string (concat "which " js-prog))))
...
;;(defun echo-true-comint () (setq comint-process-echoes nil))
;;(add-hook 'comint-mode-hook 'echo-true-comint)
...
\(Type \\[describe-mode] in the process buffer for a list of commands.)"
  ...
  ;; BSS: node and node-repl echo the command twice, rhino not at all.  Thus
  ;; we need different settings here for different modes.  Default is to
  ;; have this variable NOT turned on.
  (if (or (equal inferior-js-program-command "node-repl")
	  (equal inferior-js-program-command "node"))
      (setq comint-process-echoes t)
    )

  ;; BSS TODO: To allow node-repl and mongo to coexist, need to have
  ;; multiple names simultaneously for inferior-js-buffer.
...
;;(setq buffer-substring-filters '(remove-newlines))
...
(defun js-send-region (start end)
  ...
  ;; BSS: echo for node, and node-repl
  (if (or (equal inferior-js-program-command "node-repl")
	  (equal inferior-js-program-command "node"))
      (setq comint-process-echoes t)
    )
  ...
  ;; BSS: the shell does not like newlines. Instead, we remove all newlines and send.
  ;;(comint-send-region inferior-js-buffer start end)
  ;;(comint-send-string inferior-js-buffer (quote-js (buffer-substring start end)))
  ;;(comint-send-string inferior-js-buffer (remove-newlines (buffer-substring start end)))

  (setq curwindow (selected-window))
  (save-excursion (let ((jscode (remove-newlines (buffer-substring start end))))
		    ;;(select-window (get-buffer-window "*js*"))
		    (set-buffer "*js*")
		    (insert jscode)
		    ...
		    ;;(comint-send-input)
		    ;;(select-window (get-buffer-window "*js*"))
		    ;;(comint-send-input) ;; execute again
		    ))
  (select-window curwindow)
  ;;(comint-send-string inferior-js-buffer "\n")
  ;;(comint-send-input) ;; ...and start a new line
  ;;(comint-send-string inferior-js-buffer "\n")
  )
